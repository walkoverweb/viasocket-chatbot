...
function IconComponent() {
  return (
    <div draggable className='flex-center-center'>
      <Avatar alt='Remy Sharp' src='https://cdn2.hubspot.net/hubfs/53/image8-2.jpg' sx={{ width: 50, height: 50 }} />
    </div>
  )
}
...
function ComponentSlider({ dragRef }: ComponentSliderProps) {
  ...
  const renderComponent = ({ type }: any) => {
    return (
      <div draggable className='image_container'>
        <img src={componentIconMapping[type]} alt='RadioIcon' className='h-100' />
      </div>
    )
  }
  ...
  return (
    <Box className='component_slider box-sizing-border-box overflow-scroll-y'>
      <Grid container rowSpacing={1} columnSpacing={{ xs: 0.5, sm: 0.75, md: 1 }} spacing={1}>
        {Object.keys(elementJson).map((type) => {
          const Element = elementJson[type]
          return (
            <Grid item xs={6} key={type} className='box-sizing-border-box child_grid' onDragStart={(e) => onDragStart(e, type)}>
