...
ComponentRenderer.defaultProps = { id: '', gridId: 'root', dragRef: {}, inpreview: false }
...
function ComponentRenderer({ gridId, id: componentId, dragRef, inpreview = false, interfaceId }: ComponentRendererProps) {
  const responseTypeJson = useContext(GridContext)
  const componentData = useCustomSelector(
    (state: $ReduxCoreType) => state.Interface?.interfaceData?.[interfaceId]?.components?.[gridId]?.[componentId]
  )
  const { type, props, key, action } = componentData
  const commonProps = { ...(responseTypeJson?.[componentId]?.props || { ...props }), key }
  const component = componentMap[type] || null

  if ((component && type === 'Button') || type === 'ChatBot') {
    return component({ props: commonProps, gridId, componentId, action, inpreview })
  }

  return component ? component({ props: commonProps, gridId, componentId, action, inpreview, dragRef }) : null
}
// export default ComponentRenderer
export default React.memo(addUrlDataHoc(React.memo(ComponentRenderer), [ParamsEnums?.interfaceId]))
