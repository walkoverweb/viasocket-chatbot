...
function StepOne({ orgId, projectId, interfaceId }: StepOneProps) {
  ...
  const { embedAccessToken, interfaceAccessType } = useCustomSelector(
    (state: $ReduxCoreType) => ({
          ...
          <Box>
            <>
              {embedAccessToken ? (
                <InputCombo
                  stepName="access key"
                  value={embedAccessToken}
                  copyValue={embedAccessToken}
                />
              ) : (
                <Button
                  className="primary-btn p-2 mt-4 ml-2 mb-2"
                  variant="contained"
              ...
              />
              <InputCombo
                stepName="interface_id"
                value={interfaceId || ""}
                copyValue={interfaceId || ""}
              />
              <Box className="flex-col p-2">
                <Typography className="mt-2" variant="base">
                  Generate a JWT Token by incorporating the project_id, org_id,
                  interface_id and user_id, and subsequently sign it with the
                  access key.
                </Typography>
                <Link
                  className="learn-more-btn mt-2"
                  href="https://viasocket.com/faq/create-jwt-token"
                  target="_blank"
                >
...
export const InterfaceConfigStepOne = React.memo(
  addUrlDataHoc(React.memo(StepOne), [
    ParamsEnums?.orgId,
...
export function InterfaceConfigStepTwo() {
  return (
    <Box className="mt-4 p-3  boxShadow">
      ...
      <Link
        className="learn-more-btn ml-2"
        href="https://viasocket.com/faq/integrate-viasocket"
        target="_blank"
      >
