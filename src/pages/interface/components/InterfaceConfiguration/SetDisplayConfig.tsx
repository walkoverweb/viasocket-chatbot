...
function SetDisplayConfig({ interfaceId, projectId, inIntegration = false, orgId }) {
  ...
  const currentIntegration = useCustomSelector((state) => state.projects.embedProject[orgId]?.['active']?.[projectId] || {})
  ...
  const updateConfig = async () => {
    if (inIntegration) {
      ...
      const res = await updateInterfaceDetailsApi({ config: localConfig, interfaceId }, { projectId })
  ...
  return (
    <Box className='column w-100 gap-1  p-3 boxShadow'>
      ...
      >
        <FormControlLabel value='all_space' control={<Radio />} label='All Available space' />
        <FormControlLabel value='left_slider' control={<Radio />} label='Left slider' />
        <FormControlLabel value='right_slider' control={<Radio />} label='Right slider' />
        <FormControlLabel value='popover' control={<Radio />} label='Pop over' />
...
export default React.memo(
  addUrlDataHoc(React.memo(SetDisplayConfig), [ParamsEnums?.projectId, ParamsEnums?.interfaceId, ParamsEnums.orgId])
)
