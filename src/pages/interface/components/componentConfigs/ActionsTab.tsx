...
function ActionsTab() {
  const [isDropdownVisible, setIsDropDownVisible] = React.useState<boolean | null>(false)
  const [actionType, setActionType] = React.useState<string | null>(null)
  const { projectId, interfaceId } = getInfoParamtersFromUrl()
  ...
  const { actionsArray, frontendActionsData } = useCustomSelector((state: $ReduxCoreType) => ({
    actionsArray: state?.Interface?.interfaceData?.[interfaceId]?.actions?.[gridId]?.[componentId]?.actionsArr,
    frontendActionsData: state?.Interface?.interfaceData?.[interfaceId]?.frontendActions?.[gridId]?.[componentId]
  }))
    ...
    if (selectedActionType === 'sendDataToFrontend') {
      dispatch(
        updateInterfaceFrontendActionStart({
          ...
          frontendActions: { type: 'sendDataToFrontend', data: 'sendDataToFrontend' }
        })
      )
      setActionType(null)
    }
  ...
  return (
    <Box className='column w-100 gap-2'>
      <Box className='flex-col'>
        ...
        {Object.entries(frontendActionsData || {}).map(([key, actionElement]) => {
          return (
            <ComponentToTakePropValue
              key={key}
              propName={actionElement?.type}
      ...
      {actionType && <ComponentToTakePropValue propName={actionType} setActionType={setActionType} />}
      ...
      {!isDropdownVisible ? (
        ...
        <FormControl fullWidth>
          <Select labelId='demo-simple-select-label' id='demo-simple-select' value={actionType} displayEmpty onChange={handleChange}>
...
export default React.memo(addUrlDataHoc(React.memo(ActionsTab), [ParamsEnums?.orgId]))
