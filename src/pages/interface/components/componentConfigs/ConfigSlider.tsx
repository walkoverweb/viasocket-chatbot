...
function ComponentConfigs({ interfaceId }: ComponentConfigsProps) {
  ...
  const {
    ...
    actionChatbotData:
      state?.Interface?.interfaceData?.[interfaceId]?.actions?.["root"]?.[
        state?.Interface?.currentSelectedComponent?.componentId
      ],
    actionData:
      state?.Interface?.interfaceData?.[interfaceId]?.actions?.["root"]?.[
        state.Interface?.nestedGridSliderOpen?.componentId
      ],
  }));
        ...
        if (response.responseId === componentDetails?.gridId) {
          return {
            ...
            components: Object.fromEntries(
              Object.entries(response.components).filter(
                ([key]) => key !== componentDetails?.componentId
              )
            ),
          };
        }
  ...
  const deleteComponent = () => {
    closeSlider();
    if (componentDetails?.componentId?.includes("ChatBot"))
      deleteAllChatBotResponses();
    if (componentDetails?.gridId?.includes("Response"))
      deleteComponetFromResponse();
  ...
  const saveComponentProps = () => {
    dispatch(
      updateInterfaceStart({
        ...
        componentId: componentDetails?.componentId,
        components:
          interfaceDetails?.components?.[componentDetails?.gridId][
...
export default React.memo(
  addUrlDataHoc(React.memo(ComponentConfigs), [
