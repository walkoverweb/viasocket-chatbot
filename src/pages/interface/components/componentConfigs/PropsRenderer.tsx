...
import { Autocomplete, Box, Button, Checkbox, Divider, Fab, MenuItem, Select, TextField, Typography } from '@mui/material'
...
function ComponentToTakePropValue({
  ...
  const currentSelectedComponent = useCustomSelector((state: $ReduxCoreType) => state?.Interface?.currentSelectedComponent)
  const { gridId, componentType, componentId } = currentSelectedComponent || {}
  const actionData = useCustomSelector(
    (state: $ReduxCoreType) => state?.Interface?.interfaceData?.[interfaceId]?.actions?.[gridId]?.[componentId]
  )
  ...
  if (allowedProps[propName] === 'boolean')
    return <Checkbox checked={componentConfig[propName] || false} onChange={(e) => handleChange(e.target.checked, propName)} />
  ...
  switch (propName) {
    case 'key': {
      ...
      return (
        <Box className='flex-center-center gap-1 w-100  p-1 box-sizing-border-box'>
          <TextField value={currentSelectedComponent?.componentId || ''} fullWidth disabled />
      ...
      return (
        <Box className='w-100' key={key}>
          <Autocomplete
            ...
            renderInput={(params) => <TextField {...params} variant='standard' placeholder='Options' />}
          />
        </Box>
      )
    }
      ...
      return (
        <Box className='column gap-1 p-2 custom_styling_cont mt-'>
          <Box className='flex-center custom_styling_box'>
            ...
            <input type='color' onChange={(e) => handleChange({ ...sx, color: e.target.value }, 'sx')} />
          </Box>
          <Box className='flex-center custom_styling_box'>
            <Typography className='w-50'>Background Color</Typography>{' '}
            <input type='color' onChange={(e) => handleChange({ ...sx, backgroundColor: e.target.value }, 'sx')} />
          </Box>
          <Box className='flex-center custom_styling_box'>
            <Typography className='w-50'>Font Size</Typography>{' '}
            <span>
              <input type='number' onChange={(e) => handleChange({ ...sx, fontSize: `${e.target.value} px` }, 'sx')} />
              &nbsp;px
            </span>
          </Box>
          <Box className='flex-center custom_styling_box'>
            ...
            <span>
              <input type='number' onChange={(e) => handleChange({ ...sx, padding: `${e.target.value}px` }, 'sx')} />
              &nbsp;px
            </span>
          </Box>
          <Box className='flex-center custom_styling_box'>
            <Typography className='w-50'>font Weight</Typography>{' '}
            <input type='number' onChange={(e) => handleChange({ ...sx, fontWeight: e.target.value }, 'sx')} />
          </Box>
        </Box>
      )
    }
    ...
    case 'variant': {
      ...
      return (
        <Select value={componentConfig[propName] || variants[variants.length - 1]}>
          {variants?.map((element: string) => (
            <MenuItem key={element} value={element} onClick={() => handleChange(element, propName)}>
              {element}
            </MenuItem>
          ))}
        </Select>
    ...
    case 'textAlign': {
      ...
      return (
        <Select value={componentConfig[propName] || textAlign[textAlign.length - 1]}>
    ...
    case 'data': {
      return (
        <TextField
          ...
          onBlur={(e: any) =>
            isJSONString(e?.target?.value) ? handleChange(JSON.parse(e?.target?.value), propName) : errorToast('Please enter a valid JSON')
          }
        />
      )
    }
    ...
    case 'flow': {
      ...
      const createFlow = async () => {
        ...
        dispatch(
          updateInterfaceActionStart({
            gridId: gridId,
            componentId,
            actionsArr: [...(actionData?.actionsArr || []), { type: 'flow', eventType: 'onClick', scriptId: data.data.id }],
      ...
      const openIframeInNewTab = function (scriptId: string) {
        ...
        if (newTab) {
          ...
          console.error('Popup blocked. Please enable popups for this website.')
        }
      }
      return (
      ...
      const handleChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        dispatch(
          updateInterfaceFrontendActionStart({
            ...
            frontendActions: { type: 'navigate', url: actionElement?.url || url, openInNewTab: e?.target?.checked || false }
          })
        )
      }
      const saveUrl = (e: React.FocusEvent<HTMLInputElement>) => {
        ...
        dispatch(
          updateInterfaceFrontendActionStart({
            ...
            componentId,
            frontendActionId: actionKey || newId,
            frontendActions: { type: 'navigate', url: e?.target?.value, openInNewTab: isNavigateCheck }
          })
        )
        setActionType(null)
      }
      return (
        <Box className='flex-col gap-1 mb-3'>
          ...
          <Box className='flex-start-center'>
            <Checkbox onChange={handleChange} checked={actionElement?.openInNewTab || false} />
    ...
    case 'chatbot': {
      const savePrompt = (e: React.FocusEvent<HTMLInputElement>) => {
        ...
        const isPromptPresent = actionData?.actionsArr?.some((action: any) => action.type === 'chatbot')
      ...
      const saveApi = (e: React.FocusEvent<HTMLInputElement>) => {
        const newValue = e?.target?.value // Get the latest value
        const isApiPresent = actionData?.actionsArr.some((action: any) => action.type === 'chatbot')
    ...
    default:
      return (
        <TextField
          ...
          onChange={(e: React.ChangeEvent<HTMLInputElement>) => handleChange(e?.target?.value, propName)}
        />
      )
  }
}
...
export default React.memo(
  addUrlDataHoc(React.memo(ComponentToTakePropValue), [ParamsEnums?.orgId, ParamsEnums?.projectId, ParamsEnums?.interfaceId])
)
