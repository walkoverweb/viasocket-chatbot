...
function Interface({ interfaceId, projectId, orgId }: InterfaceProps) {
  const dispatch = useDispatch()
  const { nestedGridSliderOpen, openConfigModal, isLoading } = useCustomSelector((state: $ReduxCoreType) => ({
    ...
    <Box className='interface__page flex w-100 h-100vh box-sizing-border-box '>
      <Box className='interface__grid flex-grow h-100vh column box-sizing-border-box '>
        <div className='flex-spaceBetween-center p-2 box-sizing-border-box grid-header'>
          <Link to={`${process.env.REACT_APP_INTERFACE_URL}/i/${interfaceId}`} target='_blank'>
          ...
          <Box className='flex-start-center'>
            ...
            <Button startIcon={<SettingsIcon />} onClick={() => navigate(`/projects/${orgId}/${projectId}/${interfaceId}/interfaceSetup`)}>
              Configuration
            </Button>
          </Box>
        </div>
      ...
      {nestedGridSliderOpen && (
        <Box className='interface__child-slider'>
          <Box className='grid_slider_header flex-start-center'>
            ...
            <Button className='ml-2' variant='outlined' onClick={() => handleClick()}>
              Close
            </Button>
          </Box>
          <InterfaceForm componentId={nestedGridSliderOpen?.id} dragRef={dragRef} />
        </Box>
      )}
...
export default React.memo(addUrlDataHoc(React.memo(Interface), [ParamsEnums?.interfaceId, ParamsEnums.orgId, ParamsEnums.projectId]))
