...
export default function InterfaceEmbed() {
  ...
  const { interface_id, project_id, userId, token } = JSON.parse(queryParams.get('interfaceDetails') || '{}')
  ...
  return (
    <Box className='flex-col-center-center w-100vw h-100vh'>
      {verifiedState === EmbedVerificationStatus.VERIFYING && <CircularProgress />}
      {verifiedState === EmbedVerificationStatus.NOT_VERIFIED && <InterfaceErrorPage />}
    </Box>
  )
}
