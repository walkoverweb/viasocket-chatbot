...
// function findIndexInValueString(b, index) {
//   let result = 0
//   let boolmention = false
//   let sbstring = ''
//   for (let i = 0; i < b.length; i++) {
//     if (result === index) return i
//     if (b[i] === '$' && b[i + 1] === '{') {
//       let j = i + 1
...
//       boolmention = true
//       i++
//     } else if (b[i] === '}' && boolmention) {
//       result -=
//         sbstring.length -
//         sbstring
//           ?.replaceAll('?.[', '[')
//           ?.replaceAll('?.', '.')
//           .replace(/^context\.res\.|context\.req\.|context\.vals\.|context\./, '').length
//       sbstring = ''
//       result += 3
//       boolmention = false
//     } else {
...
//   return -1
// }
// function findSubstringAtIndex(inputString, index) {
//   // Regular expression to match substrings wrapped in ${}
//   const regex = /\${(.*?)}/g
//   let match
//   while ((match = regex.exec(inputString)) !== null) {
//     // match.index is the starting index of the matched substring
//     // match[0] is the whole matched substring including ${}
//     // match[1] is the captured content inside the braces
//     const startIndex = match.index
//     const endIndex = startIndex + match[0].length - 1
//     // Check if the given index is within the current substring
//     if (index >= startIndex && index <= endIndex) {
//       return { startIndex, word: match[0], endIndex: endIndex } // Return the matched substring including ${}
//     }
//   }

//   return -1 // Index is not within any matched substring
// }
...
export const makeTriggerRegex = function (trigger, options = {}) {
  ...
  return new RegExp(`(${escapedTriggerChar}([^${allowSpaceInQuery ? '' : '\\s'}${escapedTriggerChar}]*))$`)
}

// function findSubstringWithEmoji(str, index, emoji) {
//   // Check if the index is valid
//   if (index < 0 || index >= str.length) {
//     return null
//   }

//   // Check if the emoji is present before the given index
//   let emojiIndex = str.lastIndexOf(emoji, index)

//   if (emojiIndex === -1) {
//     return null
//   }

//   // Extract the substring starting from the found emojiIndex to the given index
//   let substring = str.substring(emojiIndex, index)

//   return { startIndex: emojiIndex, substring, endIndex: index }
// }
...
class MentionsInput extends React.Component {
  ...
  getInputProps = () => {
    ...
    // pass all props that neither we, nor substylex, consume through to the input control
    const props = omit(
      this.props,
  ...
  renderControl = () => {
    const { singleLine, style, textareaClass } = this.props
    const inputProps = this.getInputProps()
    return (
      <div {...style('control')}>
        {this.renderHighlighter()}
        {singleLine ? this.renderInput(inputProps) : this.renderTextarea({ ...inputProps, className: textareaClass })}
      </div>
    )
  }
  ...
  handlePaste(event) {
    ...
    const newValue = spliceString(value, markupStartIndex, markupEndIndex, pastedMentions || pastedData).replace(/\r/g, '')
  ...
  saveSelectionToClipboard(event) {
    ...
    const { children, value, copyText } = this.props
    ...
    event.clipboardData.setData(
      'text/plain',
      copyText ? event.target.value.slice(selectionStart, selectionEnd)?.replaceAll('🏷', '') : value.slice(markupStartIndex, markupEndIndex)
    )
  ...
  //   const valueIndex = findIndexInValueString(this.props.value, ev.target.selectionEnd)
  //   const mentionWord = findSubstringAtIndex(this.props.value, valueIndex)
  //   const config = readConfigFromChildren(this.props.children)
  //   let editMention = false
  ...
  //   let newValue = ''
  //   let textword = findSubstringWithEmoji(ev.target.value, ev.target.selectionEnd, '🏷')
  //   let wordToAppend = ''
  //   let modifiedMentionword = mentionWord?.word?.replaceAll('?.[', '[')?.replaceAll('?.', '.')
  //   if (
  //     mentionWord &&
  //     mentionWord.word &&
  //     textword?.substring &&
  //     modifiedMentionword.includes(textword?.substring?.substring(3, textword?.substring?.length - 1)) &&
  //     modifiedMentionword.substring(2, modifiedMentionword?.length - 1) !== textword?.substring?.substring(3)
  //   ) {
  //     editMention = true
  //     if (this.props.value[mentionWord.startIndex - 1] === ' ' || mentionWord.startIndex === 0) {
  //       textword.endIndex = textword.endIndex - 3
  //       wordToAppend = textword.substring.substring(3)
  //     } else {
  //       textword.endIndex = textword.endIndex - 2
  //       wordToAppend = ` ${textword.substring.substring(3)}`
  //     }
  //     newValue = this.props.value.substring(0, mentionWord.startIndex) + wordToAppend + this.props.value.substring(mentionWord.endIndex + 1)
  //   } else {
  //     newValue = applyChangeToValue(
  //       value,
  //       newPlainTextValue,
  //       {
  ...
  //   //set trigger
  //   let cursor = ev.target.selectionEnd
  //   let string = editMention ? newPlainTextValue.substring(0, cursor - 3) : newPlainTextValue.substring(0, cursor)
  //   let indexOfword = string.lastIndexOf(' ') || -1
  //   if (string.lastIndexOf('\n') > indexOfword) indexOfword = string.lastIndexOf('\n')
  //   let word = string.substring(indexOfword + 1)
  //   if (!word || word === ' ') this.props.triggerRef.current = 'c'
  //   else this.props.triggerRef.current = word.trim()
  ...
  //   if (startOfMention !== undefined && this.state.selectionEnd > startOfMention && !editMention) {
  //     // only if a deletion has taken place
  //     selectionStart = startOfMention + (ev.nativeEvent.data ? ev.nativeEvent.data.length : 0)
  //     selectionEnd = selectionStart
  //     setSelectionAfterMentionChange = true
  //   } else if (editMention) {
  //     selectionStart = textword.endIndex
  //     selectionEnd = textword.endIndex
  //     setSelectionAfterMentionChange = true
  //   }
  //   this.setState({
  //     selectionStart,
  ...
  handleChange = (ev) => {
    ...
    let cursor = ev.target.selectionEnd
    let string = newPlainTextValue.substring(0, cursor)
    let indexOfword = string.lastIndexOf(' ') || -1
    if (string.lastIndexOf('\n') > indexOfword) indexOfword = string.lastIndexOf('\n')
    const word = string.substring(indexOfword + 1)
    if (!word || word === ' ') this.props.triggerRef.current = 'c'
    else this.props.triggerRef.current = word.trim()
    // Save current selection after change to be able to restore caret position after rerendering
    let selectionStart = ev.target.selectionStart
  ...
  updateSuggestionsPosition = () => {
    ...
    if (suggestionsPortalHost) {
      ...
      if (
        (allowSuggestionsAboveCursor && top + suggestions.offsetHeight > viewportHeight && suggestions.offsetHeight < top - caretHeight) ||
  ...
  updateMentionsQueries = (plainTextValue, caretPosition) => {
    ...
    React.Children.forEach(children, (child, childIndex) => {
      ...
      if (match) {
        ...
        this.queryData(match[1], childIndex, querySequenceStart, querySequenceStart + match[1].length, plainTextValue)
      }
    })
  }
  ...
  queryData = (query, childIndex, querySequenceStart, querySequenceEnd, plainTextValue) => {
    ...
    const syncResult = provideData(
      query,
      this.updateSuggestions.bind(null, this._queryId, childIndex, query, querySequenceStart, querySequenceEnd, plainTextValue)
    )
    if (syncResult instanceof Array) {
      this.updateSuggestions(this._queryId, childIndex, query, querySequenceStart, querySequenceEnd, plainTextValue, syncResult)
    }
  }
  ...
  isOpened = () => isNumber(this.state.selectionStart) && (countSuggestions(this.state.suggestions) !== 0 || this.isLoading())
