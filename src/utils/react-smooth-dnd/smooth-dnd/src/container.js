...
function setAnimation(element, add, animationDuration = defaultOptions.animationDuration) {
  if (add) {
...
function setTargetContainer(draggableInfo, element, set = true) {
  ...
  } else if (draggableInfo.targetElement === element) {
...
function handleDrop({ element, draggables, layout, getOptions }) {
  ...
  return function (draggableInfo, indexes, forDispose = false) {
    draggablesReset()
    ...
    if (draggableInfo && !draggableInfo.cancelDrop) {
      if (draggableInfo.targetElement || getOptions().removeOnDropOut || forDispose) {
        ...
        const actualAddIndex = indexNotNull(indexes.addedIndex)
          ? indexNotNull(indexes.removedIndex) && indexes.removedIndex < indexes.addedIndex
            ? indexes.addedIndex - 1
            : indexes.addedIndex
          : null
        ...
        const dropHandlerParams = {
          removedIndex: indexes.removedIndex,
          addedIndex: actualAddIndex,
          payload,
          element: element.firstElementChild || element
        }
        const shouldHandleDrop =
          !draggableInfo.container.getOptions().fireRelatedEventsOnly || indexNotNull(indexes.removedIndex) || indexNotNull(actualAddIndex)
        if (shouldHandleDrop) {
...
function Container(element) {
  return function (options) {
    let containerOptions = { ...defaultOptions, ...options }
    let dragResult = null
    ...
    function setOptions(options, merge = true) {
      if (merge === false) {
        containerOptions = { ...defaultOptions, ...options }
      } else {
        containerOptions = { ...defaultOptions, ...containerOptions, ...options }
      }
    }
    ...
    const container = {
      ...
      fireRemoveElement() {
        ...
        dropHandler(lastDraggableInfo, { ...dragResult, addedIndex: null }, true)
        dragResult = null
      },
      getDragResult() {
        return dragResult
