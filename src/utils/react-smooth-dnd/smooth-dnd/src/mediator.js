      ...
      if ((behaviour === 'move' || behaviour === 'contain') && (isCanceling || !removeOnDropOut) && container.getDragResult()) {
        ...
        if (!Utils.isVisible(rectangles.visibleRect) && Utils.isVisible(rectangles.lastVisibleRect)) {
          ...
          const prevDraggableEnd =
            removedIndex > 0 ? layout.getBeginEnd(container.draggables[removedIndex - 1]).end : layout.getBeginEndOfContainer().begin
          animateGhostToPosition(
...
const handleDragStartConditions = (function handleDragStartConditions() {
  ...
  function onMove(event) {
    ...
    } else if (Math.abs(startEvent.clientX - currentX) > maxMoveInDelay || Math.abs(startEvent.clientY - currentY) > maxMoveInDelay) {
      deregisterEvent()
    }
  }
  function onUp() {
...
let debouncedHandleMissedDragFrame = Utils.debounce(handleMissedDragFrame, 20, false)
...
function cancelDrag() {
  if (isDragging && !isCanceling && !dropAnimationStarted) {
    isCanceling = true
    missedDrag = false
    const outOfBoundsDraggableInfo = {
      ...draggableInfo,
      targetElement: null,
